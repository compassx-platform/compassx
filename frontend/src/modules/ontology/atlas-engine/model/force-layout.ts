  ...
  return {
    tick(iterations: number, restrictToIds?: ReadonlySet<string> | null) {
      ...
      if (restrictToIds) {
        ...
        for (const id of restrictToIds) {
          ...
          graph.forEachNeighbor(id, (other: string) => {
        ...
        if (sub.order > 0) {
          ...
          sub.forEachNode((id: string, attrs: any) => {
    ...
    positions(only?: ReadonlySet<string> | null) {
      ...
      if (only) {
        ...
        graph.forEachNode((id: string, attrs: any) => put(id, attrs.x as number, attrs.y as number));
      }
