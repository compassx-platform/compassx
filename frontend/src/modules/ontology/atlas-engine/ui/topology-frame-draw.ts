...
import {
  ...
  footprintScaleFor,
  type FootprintInk,
} from "@/modules/ontology/shared/footprint-glyph";
import { DEFAULT_EXPAND } from "@/modules/ontology/shared/appearance-preferences";
import type { ExpandPreference, FootprintPreference } from "@/modules/ontology/shared/appearance-preferences";
import { isDirectionalRelation } from "@/modules/ontology/shared/knowledge-graph-utils";
...
export interface FrameDrawParams {
  ...
  /** Prop-driven expanded parents set (from Expand all toolbar toggle or URL state). */
  expandedParents?: ReadonlySet<string> | null;
}
...
export function drawTopologyFrame(params: FrameDrawParams): void {
  ...
  for (const kind of EDGE_KIND_PASSES) {
    for (let drawPos = 0; drawPos < edgeDrawOrder.length; drawPos += 1) {
      ...
      const isHoverActive = hovered || hoverTouches;
      const emphasized =
        !trailLensActive &&
        (hovered ||
      ...
      const domeEdgeExempt = emphasized || isSelectedEdge || isPathEdge || edgeEgoState === "ego" || isHoverActive;
       ...
       */
      const hoverRamp =
        hoveredNodeId !== null && !trailLensActive && !pathLensActive
          ? Math.min(1, Math.max(0, emphasisById.get(hoveredNodeId) ?? 0))
          : 0;
      const hoverLift = isHoverActive
        ? (hovered ? 1 : hoverRamp > 0.001 ? hoverRamp : (emphasisById.get(hoveredNodeId!) ?? 1))
        : 0;
      const hoverRecede =
        hoverRamp > 0 && !isHoverActive && !isSelectedEdge && !isPathEdge ? 1 - HOVER_RECEDE_ALPHA_STEP * hoverRamp : 1;
