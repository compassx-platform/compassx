...
import { NAVIGATION_INTENT_EVENT, NAVIGATION_YIELD_MS } from "@/modules/ontology/shared/navigation-intent";
...
import { DEFAULT_EXPAND, DEFAULT_MAP_ARRANGEMENT } from "@/modules/ontology/shared/appearance-preferences";
import type { CanvasBackground, ExpandPreference, FootprintPreference, GlyphSet, MapArrangement } from "@/modules/ontology/shared/appearance-preferences";
...
import { MOTION } from "@/modules/ontology/shared/motion";
...
import type { FootprintInk } from "@/modules/ontology/shared/footprint-glyph";
import {
...
export function useTopologyLoop(args: UseTopologyLoopArgs): UseTopologyLoopResult {
  ...
  const runOverviewFitRef = useRef<() => void>(() => {});
  ...
  useEffect(() => {
    ...
    // Dive the camera to the newly expanded parent, if there is one.
    // If multiple parents are expanded at once (e.g. Expand All) or all collapsed,
    // spring back to frame all revealed nodes within the screen.
    let newlyExpanded: string | null = null;
    let newlyExpandedCount = 0;
    ...
    if (newlyExpandedCount > 1 || (expandedParents.size === 0 && prev.size > 0)) {
      runOverviewFitRef.current?.();
      return;
    }
  ...
  runOverviewFitRef.current = runOverviewFit;
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      drawTopologyFrame({
        ...
        expandedParents: expandedParentsRef.current,
