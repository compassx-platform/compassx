...
function parseColorToRgb(str: string): [number, number, number] | null {
  if (!str) return null;
  const s = str.trim();
  const hexMatch = /^#?([0-9a-f]{6})$/i.exec(s);
  if (hexMatch) {
    const n = Number.parseInt(hexMatch[1], 16);
    return [(n >> 16) & 255, (n >> 8) & 255, n & 255];
  }
  const hex3Match = /^#?([0-9a-f]{3})$/i.exec(s);
  if (hex3Match) {
    const r = Number.parseInt(hex3Match[1][0] + hex3Match[1][0], 16);
    const g = Number.parseInt(hex3Match[1][1] + hex3Match[1][1], 16);
    const b = Number.parseInt(hex3Match[1][2] + hex3Match[1][2], 16);
    return [r, g, b];
  }
  const rgbMatch = /^rgba?\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)/i.exec(s);
  if (rgbMatch) {
    return [
...
/**
 * Linear interpolation between two hex or rgb colours — used only where the trail ramps
...
export function draw(ctx: CanvasRenderingContext2D, state: TraceDrawState, tokens: TraceTokens): void {
  ...
  const hasHoverLift = clamp01(state.hoverLift ?? 0);
  const emphasized = state.emphasized === true;
  ...
  if (trailWalked > 0.01 && tokens.edgeTrail) {
    ...
    width = (isDepends ? 1.8 : 1.5) - farT * 0.5 + 0.9;
  } else if (egoState === "dim") {
    if (hasHoverLift > 0.01) {
      stroke = mixHex(tokens.edgeDim, isDepends ? tokens.indigoBright : tokens.indigo, hasHoverLift);
      width = 1 + HOVER_LIFT_WIDTH_PX * hasHoverLift;
    } else {
    ...
    if (hasHoverLift > 0.01) {
      ...
      stroke = mixHex(stroke, isDepends ? tokens.indigoBright : tokens.indigo, hasHoverLift);
      width += HOVER_LIFT_WIDTH_PX * hasHoverLift;
    }
  }
  ...
  if (isDepends) {
    if (egoState === "dim" && hasHoverLift <= 0.05 && !emphasized) return;
    ...
    if (state.dependsCometEligible === false && hasHoverLift <= 0.05 && !emphasized) return;
    ...
    const ego = egoState === "ego" || state.selected === true || emphasized || hasHoverLift > 0.05;
  ...
  if ((egoState !== "ego" && hasHoverLift <= 0.05 && !emphasized) || state.containsCometEligible === false) return;
  if (state.reducedMotion === true) return;
